...
export default function Deals() {
  ...
  useEffect(() => {
    const normalizedDeals = (Array.isArray(data?.data) ? data.data : []).map(
      (deal: any) => ({
        ...
        image: deal.image ?? "",
        badge: deal.badge ?? "",
        originalPrice: deal.originalPrice ?? deal.price ?? 0,
  ...
  const filteredDeals = useMemo(() => {
    ...
    return dealList.filter((deal) => {
      return (
        ...
        deal.badge?.toLowerCase().includes(query) ||
        deal.items.some((item) => item.toLowerCase().includes(query))
      );
    });
  ...
  return (
    <>
      {dealsLoading && <DashboardLoader message="Loading deals..." />}
      {/* =====================================================
          HEADER
      ===================================================== */}
      <Toast
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-5 lg:flex-row lg:items-center lg:justify-between lg:px-6 dark:border-gray-800">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {dealList.length} {dealList.length === 1 ? "deal" : "deals"} total
            </p>
          </div>
        ...
        {filteredDeals.length === 0 && (
          <div className="flex flex-col items-center justify-center px-5 py-16 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Try changing your search or create a new deal.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl dark:bg-gray-900">
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-gray-200 px-5 py-4 sm:flex-row sm:justify-end sm:gap-3 sm:px-6 dark:border-gray-800">
              ...
              <button
                ...
                disabled={isDeleting}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-error-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-error-600 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
              >
                {isDeleting ? (
                  <>
